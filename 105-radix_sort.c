 ...
 * @array: array of values being searched for
 ...
 * @array: array of values being printed
 * @size: length of elements in array
 * @buckets: array of arrays each containing sorted members of `array`
 * @bucket_count: array containing amounts of members for arrays in `buckets`
 */
...
/**
 * radix_sort - This sorts array of integers in ascending order using a
 * Radix sort alogrithm that starts with the LSD, the 'least significant
 * (1s place) digit', and sorting by next digit to left. Size of `bucket_count`
 * here determined by max range of key variance (digits 0-9),
 * other solutions may be needed for much larger ranges.
 * @array: array of values being sorted
 * @size: length of elements in array
 */
