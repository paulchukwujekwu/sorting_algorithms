 ...
 * ascending order using an insertion sort algorithm
 * @list: is doubly linked list of integers to be sorted
 */
void insertion_sort_list(listint_t **list)
{
	...
	/* Starts dance with the 1st from house left following */
	follow = (*list);
	/* and next dancer will lead from the house right */
	lead = (*list)->next;
	while (lead)
	{
		new = lead->next;
